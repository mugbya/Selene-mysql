...
└── sdd/                    # ← 需求文档目录 (每次变动先更新需求，再实现)
```

## 开发流程

### 需求驱动开发
1. **需求优先**：每次功能变动或 bug 修复前，先在 `sdd/` 目录下更新或创建需求文档
2. **需求文档格式**：记录问题的描述、预期行为、验收标准
3. **基于需求实现**：根据需求文档进行代码实现，完成后对照需求验证
4. **可追溯**：保留需求历史，便于回溯和理解变更原因

### 需求文档命名规范
- 格式：`sdd/[序号]-[简要描述].md`
- 例如：`sdd/001-fix-table-theme-follow.md`
